...
Client.prototype._getClient = function _getClient(cb){
    ...
    for( connectionKey in self.pools.clients ){
        ...
        if (client && !client._cycle_blocked && self._constructor.validate()) {
            found = true;
            break;
        }
        if (client && client._cycle_blocked) {
            if(this._constructor.queue && self.pools.clients.length > 1){
                self._queuedClients++;
...
Client.prototype.info = function info(){
    ...
    return {
        ...
        clients : self._constructor.pools.clients.length,
