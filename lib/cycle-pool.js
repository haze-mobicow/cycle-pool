...
function Client(constructor) {
    ...
    //this._backoff = new backoff.ExponentialStrategy({
    //    initialDelay: self._constructor.retry.minTimeout || 1000,
    //    maxDelay: self._constructor.retry.maxTimeout || Infinity
    //});
...
Client.prototype._ensureClients = function _ensureClients() {
    var self = this;
    self._debug('_ensureClients');
    if(self._backingOff){
        return false;
    }
    // calculate how many clients we need to construct
    if(self._constructor.size === self.pools.clients.length){
        return true;
    }

    var sizeLeft = self._constructor.size - self.pools.clients.length;
    if(sizeLeft<=0 ){
        return false;
    }
    self._pending = sizeLeft;
...
Client.prototype._createConnection = function _createConnection() {
    var self = this;
    var call = backoff.call(self._constructor.create, function(err, con) {
        if (err) {
            if(call.getNumRetries() === self._constructor.retry.retries){
        ...
        self._onConnection(err,con);
    });
    call.setStrategy(new backoff.ExponentialStrategy({
        initialDelay: self._constructor.retry.minTimeout || 100,
        maxDelay: self._constructor.retry.maxTimeout || 1000
    }));
    call.failAfter(self._constructor.retry.retries);
    call.start();
    call.on('fail', function() {
        self._backingOff = false;
    });
};


Client.prototype._onConnection = function _onConnection(err,con) {
    var self = this;
    self._pending--;
    if(err){
        // self._backingOff = false;
        self._debug('Client Error: '+err);
        //self._created -= 1;
        return false;
    }
    self._backingOff = false;
    ...
    self._created = self.pools.clients.length;
...
Client.prototype._validate = function _validate(client){
    var self = this;
...
Client.prototype._getClient = function _getClient(cb){
    ...
    for( connectionKey in self.pools.clients ){
        ...
        if (client && !client._cycle_blocked && self._validate(client)) {
            found = true;
            break;
        }
